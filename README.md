...
ファン状態には `mode` と `state` があります。`mode` は制御方針で、`auto` は水温ベースの自動制御、`manual_on` は温度に関係なく ON 維持、`manual_off` は温度に関係なく OFF 維持です。`state` は現在の出力状態で、`on` / `off` / `disabled` / `unknown` のいずれかです。`manual_on` / `manual_off` は `fan-state.json` に保存され、明示的に `auto` へ戻すまで維持されます。
...
python -m aquapi.cli fan:auto fan_1 --config configs/aquapi.json
```

`fan:on` は `mode=manual_on`、`fan:off` は `mode=manual_off` として保存します。自動制御へ戻す場合は `fan:auto` を実行します。
...
- `GET /api/fans`
- `POST /api/fans/{fan_id}/manual-on`
- `POST /api/fans/{fan_id}/manual-off`
- `POST /api/fans/{fan_id}/auto`
...
curl http://aquapi.local:8080/api/fans
curl -X POST http://aquapi.local:8080/api/fans/fan_1/manual-on
curl -X POST http://aquapi.local:8080/api/fans/fan_1/manual-off
curl -X POST http://aquapi.local:8080/api/fans/fan_1/auto
...
`/api/tanks/latest` は水槽だけの現在状態を返します。対象は `role: "aquarium"`、`enabled: true`、`visible: true` の温度センサーです。非表示も含める場合は `include_hidden=true` を指定します。各 `tanks[]` には `display_code`、`temperature_alert_state`、`fan_id`、`fan_state`、`fan_mode`、`fan_reason` が含まれます。`status` は compact API と同じ `safety` / `warning` / `danger` / `unknown` です。

`/api/fans` はファン一覧と現在状態を返します。各 `fans[]` には `id`、`name`、`gpio`、`enabled`、`active_high`、`mode`、`state`、`reason`、`bound_tank_id`、`bound_tank_name`、`bound_tank_display_code`、`last_changed_at` が含まれます。`POST /api/fans/{fan_id}/manual-on` は `mode=manual_on`、`POST /api/fans/{fan_id}/manual-off` は `mode=manual_off`、`POST /api/fans/{fan_id}/auto` は `mode=auto` に変更します。存在しない fan id は 404、`enabled=false` の fan に対する `manual-on` は 409 を返します。API 操作も `fan-state.json` に保存され、`aquapi-collect.service` の制御ループで物理 GPIO 出力へ反映されます。
